 ...
 */
export function useWallet(connectorId) {
  ...
  const [transitionalStatus, setTransitionalStatus] = useState(null);
  ...
  const wallet = useMemo(() => {
    ...
    if (connector.chainType === ChainType.EVM) {
      ...
      const evmChain = new EvmChain(chain.name, chain);
      return new EvmWallet(address, evmChain, connector, connector.createWalletClient(evmChain));
    }
    if (connector.chainType === ChainType.SOLANA) {
      ...
      const solanaChain = new SolanaChain(chain.name, chain);
      return new SolanaWallet(address, solanaChain, connector, connector.createWalletClient(solanaChain));
    }
    return null;
