export function isValidAbi(abi: any): boolean {
  ...
  return abi.every((item) => {
    ...
    if (
      ...
      const validInputs = item.inputs.every(
        (input: any) => input && typeof input.type === 'string'
      );
      ...
      if (item.type === 'function') {
        ...
        const validOutputs = item.outputs.every(
          (output: any) => output && typeof output.type === 'string'
        );
